...
class ServerThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            if not membership_index_list:
                membership_index_list.append(1)
            try:
                self.server_socket.send(bytes(membership_index_list))
            except:
                pass
        ...
        try:
            while True:
                ...
                if data:
                    ...
                    self.socket.send(bytes(membership_index_list))
...
if __name__ == '__main__':
    ...
    """
        Connect to client and servers
    """
    client_index = 1
    while True:
        connecting_socket, connecting_address = tcp_rm_socket.accept()

        if connecting_address[1] in [7011, 7012, 7013]:
            server_thread = ServerThread(server_socket=connecting_socket)
            server_thread.start()

        else:
            client_thread = ClientThread(client_socket=connecting_socket, client_address=connecting_address,
